...
namespace Assembly {
	namespace AVX {
		namespace Scalar {
			...
			__declspec(dllexport) bool Calc_Mean_Stdev(const double* values, uint32_t arrayLength, double* mean, double* stdev);
			__declspec(dllexport) void Calc_Sphere_Area_Volume(double radius, double* area, double* volume);
			__declspec(dllexport) int32_t Compare_VCMPSD_Double(double a, double b);
			__declspec(dllexport) int32_t Compare_VCOMISD_Double(double a, double b);
