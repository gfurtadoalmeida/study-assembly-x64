...
namespace Assembly {
	namespace Core {
		namespace CPUID {
			// Must folllow the same structure defined
			// in "core_cpuid_get.asm".
			struct CpuidRegs
			{
				uint32_t EAX;
				uint32_t EBX;
			...
			public:
				...
				{
				public:
					enum class Type { Unknown, Data, Instruction, Unified };
			...
			public:
				...
				bool Has(FeatureFlag flag) const
				{
				...
				__declspec(dllexport) void LoadInfo(void);
			};
		}
	}
}